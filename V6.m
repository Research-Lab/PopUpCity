...
%x(13) = Model of WT [Superwind350(1), Mobisun 1kW(2),Mobisun 3kW(3), Mobisun 5kW(4)]
%need to introduce x(13) in the combined
%if x(13) == 1 %WT1
    %wind_speed_weibull= 0:0.5:12.5;
    W1 = (0.1645)*wind_speed.^(3)+(0.2885)*wind_speed.^(2)-1.879*wind_speed+0.0572;
    %Total_power_output = sum(W1)/1000;
    display(sum(W1)/1000, 'Total Power obtained using SuperWind350(kW)/year')
   
%elseif x(13) == 2 %WT2
    W2 = (0.011)*wind_speed.^(6)-(0.6033)*wind_speed.^(5)+(12.75)*wind_speed.^(4)-(131.99)*wind_speed.^(3)+(702.6)*wind_speed.^(2)-1740.3*wind_speed+1572.6;
    display(sum(W2)/1000, 'Total Power obtained using Mobisun 1000kW(kW)/year')

%elseif x(13) == 3 %WT2
    W3 = -(0.069)*wind_speed.^(5)+(2.3178)*wind_speed.^(4)-(27.455)*wind_speed.^(3)+(153.8)*wind_speed.^(2)-231.99*wind_speed+37.767;
    display(sum(W3)/1000, 'Total Power obtained using Mobisun 3000kW(kW)/year')   

%elseif x(13) == 4 %WT2
    W4 = -(0.1141)*wind_speed.^(5)+(3.8867)*wind_speed.^(4)-(46.667)*wind_speed.^(3)+(263.39)*wind_speed.^(2)-402.08*wind_speed+67.197;
    display(sum(W4)/1000, 'Total Power obtained using Mobisun 5000kW(kW)/year')       
%end
%end
end
